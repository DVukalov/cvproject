...
void object_counter::apply(const cv::Mat& image, cv::Mat& result, std::vector<cv::Rect> &movedAreas)
{
    ...
    if (!mBoundRect.empty())
	{
        ...
        mBoundRect.erase(std::remove_if(mBoundRect.begin(), mBoundRect.end(), [&](const cv::Rect &rect){ return rect.area() < minSize;}), mBoundRect.end());
...
void object_counter::compareArrays(std::vector<cv::Rect> &firstArray, std::vector<cv::Rect> &secondArray, std::vector<cv::Rect> &resultArray)
{
    ...
    if (firstArray.empty() || secondArray.empty()) return;
    for (std::vector<cv::Rect>::iterator firstArrayPtr = firstArray.begin();
        firstArrayPtr != firstArray.end(); ++firstArrayPtr)
	{
        for (std::vector<cv::Rect>::iterator secArrayPtr = secondArray.begin();
            secArrayPtr != secondArray.end(); ++secArrayPtr)
		{
...
{
   return  cv::Rect(std::min(rect1.x, rect2.x),
            		std::min(rect1.y, rect2.y),
            		std::max(rect1.width, rect2.width),
            		std::max(rect1.height, rect2.height));
}
